...
# ============================================================
# LẤY GỐC 100% TỪ duycrackvth.py - CHỈ CHUYỂN GIAO DIỆN CLI -> TELEGRAM
# ============================================================
...
def generate_key(key_type: str = "VIP", duration_days: int = 30) -> str:
    ...
    if key_type == "VIP":
        ...
        key = 'FREE-' + ''.join(random.choices(string.ascii_uppercase + string.digits, k=8))
...
# ==================== VTH - 40 LOGIC (GIỮ NGUYÊN 100% TỪ FILE GỐC) ====================
...
def vtd_get_result(user_id: int, secret_key: str, issue_id: int, asset: str = "BUILD") -> dict:
    ...
    try:
        resp = requests.get(VTD_RESULT_API, params={"issue": str(issue_id), "asset": asset}, headers=headers, timeout=10)
        return resp.json()
    except:
        return {}
...
{ICONS['phone']} *Mua Key: {ADMIN_USERNAME}*
""", parse_mode='Markdown')
        ...
        keyboard = types.InlineKeyboardMarkup(row_width=2)
        keyboard.add(
            types.InlineKeyboardButton(f"{ICONS['plus']} Thêm TK", callback_data='add_acc_vth'),
            types.InlineKeyboardButton(f"{ICONS['target']} Chạy VTH", callback_data='run_vth')
        )
        keyboard.add(
            types.InlineKeyboardButton(f"{ICONS['chart']} Danh sách logic", callback_data='list_vth'),
            types.InlineKeyboardButton(f"🔙 Quay lại", callback_data='back')
        )
        bot.send_message(cid, f"""
...
""", reply_markup=keyboard, parse_mode='Markdown')
        ...
        keyboard = types.InlineKeyboardMarkup(row_width=2)
        keyboard.add(
            types.InlineKeyboardButton(f"{ICONS['plus']} Thêm TK", callback_data='add_acc_vtd'),
            types.InlineKeyboardButton(f"{ICONS['target']} Chạy VTD", callback_data='run_vtd')
        )
        keyboard.add(
            types.InlineKeyboardButton(f"{ICONS['chart']} Danh sách logic", callback_data='list_vtd'),
            types.InlineKeyboardButton(f"🔙 Quay lại", callback_data='back')
        )
        bot.send_message(cid, f"""
...
""", reply_markup=keyboard, parse_mode='Markdown')
        ...
        keyboard = types.InlineKeyboardMarkup(row_width=2)
        keyboard.add(
            types.InlineKeyboardButton(f"{ICONS['plus']} Thêm TK", callback_data='add_acc_lotto'),
            types.InlineKeyboardButton(f"{ICONS['target']} Chạy LOTTO", callback_data='run_lotto')
        )
        keyboard.add(
            types.InlineKeyboardButton(f"🔙 Quay lại", callback_data='back')
        )
...
"""
        ...
        bot.send_message(cid, f"""
{ICONS['plus']} *Thêm tài khoản {game}*

Dùng lệnh: `/add_acc <userId> <secretKey>`
...
""", parse_mode='Markdown')
        ...
        if game == 'VTH':
            text = f"{ICONS['chart']} *Logic VTH (40):*\n\n"
            for i, (k, v) in enumerate(VTH_LOGIC_DESCRIPTIONS.items(), 1):
                text += f"{i}. {v}\n"
        elif game == 'VTD':
            text = f"{ICONS['chart']} *Logic VTD (35):*\n\n"
            for i, (k, v) in enumerate(VTD_LOGIC_DESCRIPTIONS.items(), 1):
                text += f"{i}. {v}\n"
        else:
            text = f"{ICONS['chart']} *Logic LOTTO (10):*\n\n"
...
""", parse_mode='Markdown')
    ...
    # Kiểm tra key có trong valid_keys không
