...
def init_user(chat_id):
    if chat_id not in user_states:
        user_states[chat_id] = {'bet_amount': 10000, 'multiplier': 2, 'running': False, 'account': None}

# ========================================================================
# THUẬT TOÁN - CHUYỂN T/X THÀNH LỚN/NHỎ
# ========================================================================
...
def set_bet(message):
    ...
    try:
        amount = int(parts[1])
        if amount < 1000:
            bot.reply_to(message, "⚠️ Tối thiểu 1,000 BUILD")
            return
        cid = message.chat.id
        init_user(cid)
...
def run_lotto(chat_id):
    ...
    bet_amount = state.get('bet_amount', 10000)
            ...
            if len(history) >= 3:
                ...
                if pred:
                    ...
                    if result.get('code') == 0:
                        bot.send_message(chat_id, f"🎯 {pred['prediction']} | 💰 {current_bet:,} | 🔥 {pred['confidence']}%")
                        current_bet = int(current_bet * 2)
                        win_count += 1
                    else:
                        lose_count += 1
                        current_bet = bet_amount * 3
            time.sleep(5)
        except Exception as e:
            logger.error(f"Lotto error: {e}")
            time.sleep(5)
    bot.send_message(chat_id, f"🛑 Đã dừng! ✅ {win_count} ❌ {lose_count}")
